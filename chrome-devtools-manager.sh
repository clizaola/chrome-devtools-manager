...
show_help() {
  ...
  echo "  $CMD_NAME --add [port]             Add chrome-devtools to current project's .mcp.json"
  echo "  $CMD_NAME --remove                 Remove chrome-devtools from current project's .mcp.json"
  echo ""
  echo "Registry:"
  echo "  $CMD_NAME --list                   Show port registry"
  echo "  $CMD_NAME --scan                   Rescan search paths and rebuild the registry"
  echo "  $CMD_NAME --path <dir>             Add a search path (e.g. ~/Projects/*)"
  echo ""
  echo "Diagnostics and cleanup:"
  echo "  $CMD_NAME --doctor                 Non-destructive health check (stale, drift,"
  echo "                                                conflict, wrong-port, dead, orphan)"
  echo "  $CMD_NAME --clean                  Delete current project's /tmp/chrome-<project>"
  echo "  $CMD_NAME --clean --force          Delete current project's profile, killing Chrome if active"
  echo "  $CMD_NAME --clean --all            Delete every /tmp/chrome-* profile (skips active)"
  echo "  $CMD_NAME --clean --all --force    Kill every active Chrome and wipe every profile"
  echo ""
  ...
  echo "  1. $CMD_NAME --scan              Index existing projects"
  echo "  2. cd ~/my-project"
  echo "  3. $CMD_NAME --add               Add chrome-devtools (auto-assigns next free port)"
  echo "  4. $CMD_NAME                     Launch Chrome for this project"
  echo ""
  echo "When something feels off:"
  echo "  $CMD_NAME --doctor               Diagnose the whole setup in one shot"
  echo ""
  echo "Adding custom search paths:"
  echo "  $CMD_NAME --path ~/Projects/*    Single level"
  echo "  $CMD_NAME --path ~/Work/*/*      Nested (org/project)"
  echo ""
  echo "Shell alias for .zshrc/.bashrc:"
  echo "  alias cdm='$CMD_NAME'"
  echo ""
  echo "Requires: jq (for --add and --remove). Optional: curl (enables launch verification"
  echo "and --doctor liveness checks). Install: brew install jq  /  sudo apt install jq"
  echo "Chrome DevTools MCP: https://github.com/ChromeDevTools/chrome-devtools-mcp"
  echo "Source and issues:   https://github.com/clizaola/chrome-devtools-manager"
}
